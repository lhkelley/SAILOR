...
We use ```markdup``` from the ```samtools``` package to do this. Below is a loop to loop through your BAM files. The ```-r``` flag tells the program to remove (rather than just mark) the duplicates and the ```-s``` flag prints out the statistics/summary of the run.
...
```
In the ```mpileup``` command above, the ```-a``` flag adds annotation to the output file and ```DP4``` means that the command is including both forward and reverse for both the reference and the alternate annotation.

The ```-I``` flag tells the command to ignore indels and only include SNPs. Remove this flag if you want to include indels.
